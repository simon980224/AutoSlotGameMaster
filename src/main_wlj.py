...
class Constants:
    ...
    ERROR_REMIND_CENTER_Y: float = 0.625
    ...
    # 免費遊戲類別名稱映射
    # =========================================================================
    FREE_GAME_TYPE_NAMES: Dict[Optional[int], str] = {
        None: "預設",
        1: "免費遊戲",
        2: "覺醒之力",
        3: "不朽覺醒",
    ...
    #     42, 48, 54, 56, 60, 64, 72, 80, 96, 100, 112, 120, 128, 140, 144, 160, 180, 200,
    #     240, 280, 300, 320, 360, 400, 420, 480, 500, 540, 560, 600,
    #     640, 700, 720, 800, 840, 900, 960, 980, 1000, 1080, 1120,
...
class ProxyConnectionHandler:
    ...
    def _inject_auth_header(self, request: bytes) -> bytes:
        """在請求中注入代理認證標頭。
    ...
    def handle_connect_request(
        ...
        try:
            ...
            new_request = self._inject_auth_header(request)
...
class GameControlCenter:
    ...
    def _execute_on_active_browsers(
        ...
        with ThreadPoolExecutor(max_workers=len(active_browsers)) as executor:
            futures = {
                executor.submit(task_func, bt.context): bt
    ...
    def _parse_browser_indices(
        self,
        arguments: str,
        all_browsers: Optional[List['BrowserThread']] = None
    ) -> Optional[List['BrowserThread']]:
        """解析瀏覽器編號參數並返回對應的瀏覽器列表。

        支援格式:
            - ``"0"``: 選擇所有瀏覽器
            - ``"1"``: 選擇第 1 個瀏覽器
            - ``"1,2,3"``: 選擇第 1、2、3 個瀏覽器

        參數:
            arguments: 編號字串。
            all_browsers: 可選的候選瀏覽器列表（預設使用活躍瀏覽器）。

        回傳:
            匹配的瀏覽器列表，解析失敗返回 None。
        """
        if all_browsers is None:
            all_browsers = [
                bt for bt in self._get_active_browsers()
                if bt.is_browser_alive() and bt.context
            ]
    ...
    def _recovery_image_detection_flow(self, bt: 'BrowserThread') -> bool:
        ...
        while detected_template is None and attempt < max_attempts:
            ...
            try:
                ...
                if game_confirm_exists:
                    result = self._image_detector.detect_in_browser(
                        driver, Constants.GAME_CONFIRM
                    )
                    if result is not None:
                        detected_template = Constants.GAME_CONFIRM
                        continue
                ...
                if error_remind_exists:
                    ...
                    if result is not None:
                        detected_template = Constants.ERROR_REMIND
                        continue
        ...
        if detected_template == Constants.GAME_CONFIRM:
            ...
            # 等待圖片消失（參照初始登入流程）
            self._recovery_wait_for_image_disappear(bt, Constants.GAME_CONFIRM, display_name)
    ...
    def _recovery_wait_for_image_disappear(
        self,
        bt: 'BrowserThread',
        template_name: str,
        display_name: str
    ) -> bool:
        """恢復流程：持續等待圖片在單一瀏覽器中消失（參照初始登入流程）。
        
        若圖片未消失，會重新點擊對應按鈕後再繼續等待。
        ...
        while True:
            ...
            try:
                # 直接操作 driver 檢測圖片是否仍存在
                driver = bt.context.driver
                result = self._image_detector.detect_in_browser(
                    driver, template_name
                )
                still_present = result is not None
                
                if not still_present:
                    self.logger.info(
                        f"瀏覽器 {browser_index} ({username}) {display_name} 已消失"
                    )
                    return True
                    
            except Exception as e:
                self.logger.debug(
                    f"瀏覽器 {browser_index} 等待圖片消失時發生錯誤: {e}"
                )
            
            # 每 10 次顯示一次進度
            if attempt % Constants.DETECTION_PROGRESS_INTERVAL == 0:
                self.logger.info(
                    f"瀏覽器 {browser_index} 等待 {display_name} 消失中... "
                    f"(已嘗試 {attempt} 次)"
                )
            
            # 每 20 次嘗試重新點擊一次按鈕（防止點擊未生效）
            if attempt % 20 == 0:
                self.logger.info(
                    f"瀏覽器 {browser_index} {display_name} 仍存在，重新點擊..."
                )
                # 根據模板名稱決定點擊座標
                if template_name == Constants.GAME_CONFIRM:
                    self._recovery_click_button(
                        bt,
                        Constants.GAME_CONFIRM_BUTTON_X_RATIO,
                        Constants.GAME_CONFIRM_BUTTON_Y_RATIO,
                ...
                elif template_name == Constants.GAME_LOGIN:
                    self._recovery_click_button(
                        bt,
                        Constants.GAME_LOGIN_BUTTON_X_RATIO,
                        Constants.GAME_LOGIN_BUTTON_Y_RATIO,
    ...
    def _recovery_click_button(
        ...
        for click_attempt in range(Constants.MAX_RETRY_ATTEMPTS):
            try:
                ...
                self.logger.debug(
                    f"瀏覽器 {browser_index} 已點擊 {display_name} "
    ...
    def _recovery_detect_and_click(
        ...
        # 點擊（包含重試機制）
        return self._recovery_click_button(bt, x_ratio, y_ratio, display_name)
    
    def _recovery_click_error_confirm(self, bt: 'BrowserThread') -> bool:
        """恢復流程：點擊錯誤訊息確認按鈕（直接操作 driver）。"""
        try:
            driver = bt.context.driver
...
"""
    ...
    def _get_free_game_type_name(self, free_game_type: Optional[int]) -> str:
        """取得免費遊戲類別名稱。"""
        return Constants.FREE_GAME_TYPE_NAMES.get(free_game_type, f"未知({free_game_type})")
    ...
    def _execute_auto_spin_for_all(self, spin_count: int) -> None:
        ...
        """
        # 根據次數選擇對應的座標比例
        count_x_ratio, count_y_ratio = Constants.AUTO_SPIN_COUNT_RATIOS.get(spin_count, (0.5, 0.5))
    ...
    def _handle_quit_command(self, arguments: str) -> bool:
        ...
        active_browsers = self._get_active_browsers()
        target_browsers = self._parse_browser_indices(arguments, active_browsers)
    ...
    def _handle_free_game_command(self, arguments: str) -> None:
        ...
        # 解析目標瀏覽器編號
        target_browsers = self._parse_browser_indices(arguments)
    ...
    def start(self) -> None:
        ...
        self.logger.info("")
        self.logger.info(f"[計時] 將在 {delay_minutes} 分鐘後自動執行 '{Constants.AUTO_START_COMMAND}' 命令")
        self.logger.info("   如需取消，請輸入任意命令")
        self.logger.info("")
        ...
        def auto_execute_command() -> None:
            ...
            if not self._user_has_input and self.running:
                self.logger.info("")
                self.logger.info(f"[計時] {delay_minutes} 分鐘已到，自動執行 '{Constants.AUTO_START_COMMAND}' 命令...")
                self.logger.info("")
...
class AutoSlotGameAppStarter:
    ...
    def start_control_center(self) -> None:
        ...
        # 嘗試取得 Canvas 區域資訊（直接操作 driver）
        canvas_rect = None
        for bt in self.browser_threads:
            if bt.context and bt.is_browser_alive():
                try:
                    result = BrowserHelper.get_canvas_rect(bt.context.driver)
                    if result:
                        canvas_rect = result
                        break
                except Exception:
