...
**問題**: 登入表單和公告使用同一個 HTML 結構（`.popup-wrap, .popup-account-container`），系統無法區分是登入失敗還是登入成功後出現公告，導致誤判和無效重試

**核心改進**:

1. **智能彈窗類型識別**：
   - 檢查彈窗中所有 `<span>` 元素的文字內容
   - 檢測關鍵字：「公告」、「通知」、「活動」、「最新消息」
   - 同時檢查是否存在帳號和密碼輸入框
...
2. **三種彈窗處理策略**：
   ...
   if is_announcement and not is_login_form:
       # 公告彈窗 → 關閉 → 繼續檢查（不計入重試次數）
       close_popup()
       continue

   elif is_login_form:
       # 登入表單 → 重新輸入帳號密碼 → 重試（最多 3 次）
       retry_login()
       login_attempt += 1
...
3. **精準判斷邏輯**：
   ...
   # 檢查是否為公告
   is_announcement = any(
       keyword in span.text for span in spans
       for keyword in ["公告", "通知", "活動", "最新消息"]
   )

   # 檢查是否為登入表單
   is_login_form = (
       popup.find_element(USERNAME_INPUT) and
       popup.find_element(PASSWORD_INPUT)
   )
   ```

**優勢**:

- **避免誤判**：不再將登入後的公告誤認為登入失敗
- **節省資源**：公告彈窗直接關閉，不執行無效的登入重試
- **提升成功率**：只有真正的登入失敗才執行重試，大幅提升穩定性
- **靈活擴展**：支援多種公告關鍵字，可根據實際情況調整
...
存在彈窗？
    ├─ 是 → 分析彈窗類型
    │       ├─ 檢查 span 內容（公告關鍵字）
    │       └─ 檢查輸入框（USERNAME_INPUT + PASSWORD_INPUT）
    │
    │       判斷類型：
    │       ├─ 公告（有關鍵字 + 無輸入框）
    │       │   → 點擊關閉按鈕 → continue（不計入重試）
    │       │
    │       ├─ 登入表單（有輸入框）
    │       │   → 重新輸入帳號密碼 → 重試 → login_attempt++
    │       │
    │       └─ 未知彈窗
    │           → 嘗試關閉 → 成功則 continue，失敗則 break
    │
    └─ 否 → 登入成功 ✓
```

**相關程式碼**: `SyncBrowserOperator.perform_login_all()` - [main.py#L1947-L2040]
